...
export default function SalesOrders() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const { data } = await api.get('/sales-orders', { params: { status, search } })
      setRows(data.data)
    } finally {
      setLoading(false)
    }
  ...
  const columns = [
    { key: 'number', label: t('number'), render: (r) => <Link to={`/sales-orders/${r.id}`} className="text-blue-600 dark:text-blue-400 hover:underline">{r.number}</Link> },
    ...
    <div>
      <PageHeader
        ...
        actions={
          <>
            <Select
              options={[{ value: '', label: t('all_status') }, ...['draft', 'confirmed', 'delivered', 'cancelled'].map((s) => ({ value: s, label: t(s) }))]}
              value={status}
              onChange={(e) => setStatus(e.target.value)}
              className="w-44"
            />
            <Input placeholder={t('search')} value={search} onChange={(e) => setSearch(e.target.value)} className="w-56" />
            <Button onClick={() => navigate('/sales-orders/new')}>+ {t('new')}</Button>
          </>
        }
      />
      <Card>
        {loading ? <Spinner /> : <Table columns={columns} data={rows} onRowClick={(r) => navigate(`/sales-orders/${r.id}`)} />}
      </Card>
    </div>
  )
}
