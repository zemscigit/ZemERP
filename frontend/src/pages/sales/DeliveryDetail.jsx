      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <Card title={t('document')} className="lg:col-span-2">
          <div className="grid grid-cols-2 gap-4 text-sm mb-4">
            ...
            {doc.sales_order && (
              <div>
                <p className="text-gray-500 dark:text-gray-400">{t('sales_orders')}</p>
                <button onClick={() => navigate(`/sales-orders/${doc.sales_order.id}`)} className="text-blue-600 dark:text-blue-400 hover:underline">{doc.sales_order.number}</button>
              </div>
            )}
          </div>
        ...
        <div className="space-y-4">
          <Card title={t('summary')}>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between"><span className="text-gray-500 dark:text-gray-400">{t('items')}</span><span className="text-gray-700 dark:text-gray-300">{doc.items.length}</span></div>
              <div className="flex justify-between"><span className="text-gray-500 dark:text-gray-400">{t('total_amount')}</span><Money value={doc.items.reduce((s, i) => s + i.amount, 0)} /></div>
            </div>
          </Card>
