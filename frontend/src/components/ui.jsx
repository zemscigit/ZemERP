...
export function Select({ label, options = [], className = '', placeholder, value, onChange, disabled }) {
  ...
  // ขยับตาม scroll/resize ขณะเปิด (ใช้ fixed positioning เพื่อไม่ถูกตัดโดย overflow container)
  useEffect(() => {
    if (!open) return
  ...
  return (
    <div className={`relative ${className}`} ref={rootRef}>
      ...
      <button
        ...
        onClick={toggle}
        aria-expanded={open}
        className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm bg-white flex items-center justify-between gap-2 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
      >
      ...
      {open && pos && (
        <div
          className="fixed z-50 bg-white rounded-lg border border-gray-200 shadow-lg overflow-hidden flex flex-col"
        ...
        >
          ...
          <ul className="overflow-y-auto">
            {filtered.length === 0 && <li className="px-3 py-2.5 text-sm text-gray-400">{t('no_data')}</li>}
            {filtered.map((o) => {
                  ...
                  >
                    <span className="flex items-center gap-2 min-w-0">
                      {o.image ? (
                        ...
                        <span className="w-6 h-6 rounded bg-gray-100 flex items-center justify-center text-xs shrink-0">📦</span>
                      )}
                      <span className="truncate">{o.label}</span>
                    </span>
                    {active && <span className="text-blue-600 shrink-0">✓</span>}
                  </button>
                </li>
              )
            })}
